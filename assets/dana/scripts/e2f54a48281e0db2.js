...
(()=>{
 ...
 if(!section||typeof select!=='function')return;
 ...
 let enabled=false,step=0,raf=0,lastIdx=-1;
 function update(){raf=0;if(!enabled)return;
  ...
  if(index!==lastIdx){lastIdx=index;select(tabs[index]);}
  section.querySelector('.feature-count').textContent=String(index+1).padStart(2,'0')+' / COMMAND CENTRE';
 }
 function setup(){
